...
y_test = [100, 200, 150, 120, 180]  # Replace with actual y_test values
y_pred = [110, 190, 145, 125, 175]  # Replace with predicted y_pred values
...
def predict(request, ticker_value, number_of_days):
    ...
    # Convert number_of_days to an integer
    try:
        number_of_days = int(number_of_days)
    except ValueError:
        return render(request, 'Invalid_Days_Format.html', {})

    print(f"Received ticker: {ticker_value}, number of days: {number_of_days}")
    ...
    # Fetch market data
    try:
        df = yf.download(tickers=ticker_value, period='3mo', interval='1h')
        if df.empty:
            return render(request, 'Invalid_Ticker.html', {})
    ...
    fig.update_layout(title='{} Live Share Price Evolution'.format(ticker_value),
    ...
    try:
        ...
        X = preprocessing.scale(X[:-forecast_out])  # Scale only the training data
        ...
        print(f"Error in machine learning process: {e}")
        return render(request, 'API_Down.html', {})
    '''tickers = pd.read_csv('app/Data/Tickers.csv')['Ticker'].tolist()
    ...
    for ticker in tickers:
        try:
            ...
            df_ml = yf.download(tickers=ticker, period='3mo', interval='1h')
            df_ml = df_ml[['Adj Close']]

            # Define forecast parameters
            forecast_out = 5  # Example: predicting 5 steps ahead
            df_ml['Prediction'] = df_ml[['Adj Close']].shift(-forecast_out)

            # Prepare data for training
            X = np.array(df_ml.drop(['Prediction'], axis=1))
            X = preprocessing.scale(X[:-forecast_out])  # Scale only training data
            y = np.array(df_ml['Prediction'])[:-forecast_out]
    ...
    # Classification Models (For each ticker, you can apply classification models if needed)
    ...
    for ticker in tickers:
        try:
            ...
            # Evaluate using classification models (optional if classification is needed)
            for model_name, model in classification_models.items():
                ...
                results.append([ticker, model_name, accuracy, precision, recall, f1, '-', '-', '-'])

            # Train and evaluate using regression model (Linear Regression)
            clf = LinearRegression()
            clf.fit(X_train, y_train)
            y_pred = clf.predict(X_test)
            ...
            # Append results for regression
            results.append([ticker, "Linear Regression", '-', '-', '-', '-', mae, mse, r_squared])

        except Exception as e:
            print(f"Error processing ticker {ticker}: {e}")

    # Create results DataFrame for classification and regression
    columns = ["Ticker", "Model", "Accuracy", "Precision", "Recall", "F1 Score", "MAE", "MSE", "R-squared"]
    ...
    pred_fig.update_layout(title='Predicted Stock Prices',
                           paper_bgcolor="#14151b", plot_bgcolor="#14151b", font_color="white")
    plot_div_pred = plot
    Valid_Ticker = [
